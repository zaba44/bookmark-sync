...
// ---- ClientInfo — persystentne dane klientów ----
...
public class AppConfig
{
    ...
    [JsonPropertyName("clientsInfo")]     public List<ClientInfo>      ClientsInfo        { get; set; } = [];
    ...
    public ClientInfo? GetClientInfo(string instanceId) =>
        ClientsInfo.FirstOrDefault(c => c.InstanceId == instanceId);

    public void SaveClientInfo(string instanceId, string displayName, string profileId)
    {
        var existing = ClientsInfo.FirstOrDefault(c => c.InstanceId == instanceId);
        if (existing != null)
        {
        ...
        {
            ClientsInfo.Add(new ClientInfo { InstanceId = instanceId, DisplayName = displayName, ProfileId = profileId });
        }
        Save();
    }
