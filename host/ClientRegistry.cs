...
public class ClientRegistry
{
    ...
    {
        _config = config;
        // Załaduj znanych klientów z config — z zapisanymi nazwami
        foreach (var id in config.AllowedClients)
        {
            var info = config.GetClientInfo(id);
            _clients[id] = new ConnectedClient
            {
                InstanceId  = id,
                DisplayName = info?.DisplayName ?? id[..Math.Min(8, id.Length)],
                ProfileId   = info?.ProfileId   ?? config.DefaultProfile?.Id ?? "",
            };
        }
    }
    ...
    public ClientStatus RegisterClient(string instanceId, string browserName)
    {
        lock (_lock)
        {
            ...
            if (allowed)
            {
                ...
                _config.SaveClientInfo(instanceId, displayName, profileId);
                return ClientStatus.Allowed;
            }
            else
            {
